...
parser.add_argument(
    "inpath",
    help="file or path (recursively) to be searched for crushable pngss",
    metavar="path",
    nargs="+",
    type=str,
)
...
def optimize_image(image):
    ...
    if (os.path.getsize(image) > size_initial) or (
        perc_delta * -1 < THRESHOLD
    ):  # got bigger, or exceeds threshold
        with open(image, "wb") as f:  # write back original file
            f.write(initial_file_content)
    else:
        print(
            "optimized  {image}  from {size_initial} to {size_after}, {size_delta}b, {perc_delta}%".format(
                image=image,
